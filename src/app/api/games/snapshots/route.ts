  ...
  try {
    ...
    if (snapshotId) {
      const { data, error } = await supa
        ...
        .maybeSingle();
      if (error) {
        if (isMissingRelationError(error)) return NextResponse.json({ snapshot: null });
    ...
    if (error) {
      if (isMissingRelationError(error)) return NextResponse.json({ snapshots: [] });
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
  ...
  try {
    ...
    if (error) {
      if (isMissingRelationError(error)) {
        return NextResponse.json({ snapshot: null, skipped: true });
      }
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
