...
export function MapCommandHud({ origin, dest, hubCode, activeRouteCount, compact }: MapCommandHudProps) {
  ...
  // Auto-hide once the player launches their first route — they don't
  // need the route-flow tutorial anymore and the floating panel just
  // clutters the map. They can still re-open by going through the
  // launch flow again (eventually we'll add a help icon in the rail).
  // Threshold matches feat/skyforce-v1's f804bfb intent: hide as soon
  // as one live route exists.
  const autoHidden = (activeRouteCount ?? 0) >= 1;
